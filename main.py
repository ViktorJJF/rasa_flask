...
def get_agent(bot_id):
    agent = Agent.load(
        f"./models/{bot_id}.tar.gz", action_endpoint=EndpointConfig(action_endpoint))
...
def trainModel():
    ...
    train(f"./bots/{bot_id}/domain.yml",f"./config.yml", f"./bots/{bot_id}/data","./models",fixed_model_name=f"{bot_id}")
...
def createBaseSkillTrained():
    ...
    # load agent
    get_agent(bot_id)
    return jsonify({"ok":True,"msg":f"Bot skill base para bot: {bot_id} creado y entrenado"})
...
def new_message():
    ...
    message = asyncio.run(current_agent.handle_text(text_message=input, sender_id=sender_id))
    ...
    predict_next_for_sender_id=asyncio.run(current_agent.predict_next_for_sender_id(sender_id=sender_id))
